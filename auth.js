...
(function () {
  ...
  style.textContent = `
    ...
    #auth-widget .auth-email {
      font-family: 'DM Mono', monospace; font-size: 0.75rem; color: #6b7a99;
      max-width: 140px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
    }
    @media (max-width: 640px) {
      #auth-widget .auth-email { display: none; }
    }
  ...
  window.saveExamAttempt = async function (examName, bankKey, correct, total, mode, wrong) {
    ...
    try {
      ...
      console.warn('If this is a "Missing or insufficient permissions" error, your Firestore security rules are not published yet — see ADDING_A_NEW_QUIZ.md or the Firebase Console → Firestore Database → Rules.');
    }
  };
  ...
  window.getMyHistory = async function () {
    const user = auth.currentUser;
    if (!user) return [];
    // Sorting client-side (rather than .orderBy() in the query) avoids needing
    // a Firestore composite index for this where+order combination.
